...
void VolumetricMaterial::setUniforms(Camera* camera, Matrix44 model) {
	...
	if (noise_tex) {
		...
		shader->setUniform("u_noise_size", (float) noise_tex->image.width);
	}
	...
	shader->setUniform("u_version", (float)algo_version);
	// Additional ImGui Adjustments
	shader->setUniform("u_reduction_factor", (float)reduction_factor);
	shader->setUniform("u_color_adjustment", (float)color_adjustment);
	shader->setUniform("u_volume_id", (float)volume_id);
	// Transfer Function (TF) texture
	if (volume_id == 0) {
		shader->setUniform("u_tf_texture", Texture::Get("data/volumes/tf_bonsai.png"));
	}
	if (volume_id == 1) {
		shader->setUniform("u_tf_texture", Texture::Get("data/volumes/tf_teapot.png"));
	}
	if (volume_id == 2) {
		shader->setUniform("u_tf_texture", Texture::Get("data/volumes/tf_feet.png"));
	}
...
void VolumetricMaterial::renderInMenu() {
	...
	ImGui::SliderFloat("Reduction factor: ", &reduction_factor, 1.0f, 3.0f);
	ImGui::SliderFloat("Color adjustment: ", &color_adjustment, 0.01f, 0.2f);
	ImGui::Combo("ALGORITHM", (int*)&algo_version, "V0 (part 1, OLD, Black-White)\0V1 (part 1, NEW, Green-Red Classification and Composition)\0V2 (part 2, TF (hardcoded))\0V3 (part 2, TF (texture))\0ISOSURFACES");
	ImGui::Combo("Transfer Function", (int*)&volume_id, "BONSAI\0TEAPOT\0FEET\0");
}